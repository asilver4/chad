...
def test_recheck_spiral():
    """The post-recheck edit cap trips only once landed fix edits exceed
...
def test_done_audit(monkeypatch):
    ...
    recheck restored — when ablated) lives in test_done_audit.py's e2e pair. Here assert
    the gate itself flips, so the coverage contract stays honest."""
    n = bite("done_audit")
    on(monkeypatch)
    assert levers.enabled(n)
...
def test_turn_think_budget(monkeypatch):
    """The budget's behavioral bite (persistent no-think once the cumulative spend is
    exhausted, gone when ablated) lives in test_agent_e2e.py's e2e pair. Here assert the
    threshold latch itself only fires with the lever on."""
    n = bite("turn_think_budget")
    on(monkeypatch)
    assert levers.enabled(n)
    decision, half, exhausted = guardrails.turn_think_budget_check(8000, 8000, False, False)
    assert decision == "exhausted" and half and exhausted
    off(monkeypatch, n)
    assert not levers.enabled(n)


# === 103 hard wrap-up abort =====================================

def test_hard_wrapup(monkeypatch):
    """The abort's behavioral bite (a wall-clock stop_condition that cuts an in-flight
    generation, then a boxed no-think landing turn) lives in the run_turn loop; here assert
    the arming gate flips: with the lever on a wall budget yields a land_margin, ablated it
    arms nothing so a slow generation rides to the SIGKILL."""
    n = bite("hard_wrapup")
    on(monkeypatch)
    assert levers.enabled(n)
    assert guardrails.hard_wrapup_deadline(900, False, False, False) == 90.0
    off(monkeypatch, n)
    assert not levers.enabled(n)
    assert guardrails.hard_wrapup_deadline(900, False, False, False) is None
