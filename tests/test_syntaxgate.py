"""Unit tests for the deterministic post-edit syntax gate (syntaxgate.py).
...
def test_python():
    ...
    # untouched — write was the warn-only escape hatch that delivered 51 of
    ...
    # An edit that breaks the file is REJECTED and reverted: a landed break
...
def test_tree_sitter_delta():
    ...
    # A clean C file that an edit would break is REJECTED and reverted (the
    # measured non-Python class — these breaks used to land with a warning and
